#pragma once
//========================================================================
// RevolverWarsView.h : source file for the sample game
//
...
class RevolverWarsHumanView : public HumanView
{
protected:
	bool  m_bShowUI;					// If true, it renders the UI control text
    std::wstring m_gameplayText;

	shared_ptr<RevolverController> m_pRevolverController;
	shared_ptr<MovementController> m_pFreeCameraController;
	shared_ptr<SceneNode> m_pRevolver;
	shared_ptr<StandardHUD> m_StandardHUD; 

public:
	RevolverWarsHumanView(shared_ptr<IRenderer> renderer); 
	virtual ~RevolverWarsHumanView();
...
class AIRevolverView : public IGameView 
{
	friend class AIRevolverViewListener;
...
public:
	AIRevolverView(shared_ptr<PathingGraph> pPathingGraph);
	virtual ~AIRevolverView();
