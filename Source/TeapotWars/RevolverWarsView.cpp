...
#include "RevolverController.h"
#include "RevolverWars.h"

extern RevolverWarsApp g_RevolverWarsApp;
...
MainMenuUI::MainMenuUI()
{
	...
	m_SampleUI.AddStatic(0, L"Revolver Wars Main Menu", iX-30, iY, g_SampleUIWidth, height * 2);
	iY += (lineHeight * 1.5);

	m_SampleUI.AddStatic(0, L"Objective: Win the duel in the wild west!", width / 2 - 30, iY, g_SampleUIWidth, height * 2);
	iY += (lineHeight * 1.5);
...
{
	HRESULT hr;
	DXUT_BeginPerfEvent( DXUT_PERFEVENTCOLOR, L"RevolverWarsHUD" ); // These events are to help PIX identify what the code is doing
...
//========================================================================
//
// RevolverWarsHumanView Implementation
//
//========================================================================

//
// RevolverWarsHumanView::RevolverWarsHumanView	- Chapter 19, page 724
//
RevolverWarsHumanView::RevolverWarsHumanView(shared_ptr<IRenderer> renderer) : HumanView(renderer)
{ 
	m_bShowUI = true; 
...
RevolverWarsHumanView::~RevolverWarsHumanView() 
{
    RemoveAllDelegates();
}

//
// RevolverWarsHumanView::VOnMsgProc				- Chapter 19, page 727
//
LRESULT CALLBACK RevolverWarsHumanView::VOnMsgProc( AppMsg msg )
{
	if (HumanView::VOnMsgProc(msg))
		return 1;
	...
	if (msg.m_uMsg==WM_KEYDOWN)
	{
		...
		{
			RevolverWarsLogic *twg = static_cast<RevolverWarsLogic *>(g_pApp->m_pGame);
			twg->ToggleRenderDiagnostics();

		}
		else if (msg.m_wParam==VK_F9)
		{
			m_KeyboardHandler = m_pRevolverController;
			m_PointerHandler = m_pRevolverController;
			m_pCamera->SetTarget(m_pRevolver);
			m_pRevolver->SetAlpha(0.8f);
			ReleaseCapture();
			return 1;
		}
...
//
// RevolverWarsHumanView::VRenderText				- Chapter 19, page 727
//
void RevolverWarsHumanView::VRenderText()
{
...
//
// RevolverWarsHumanView::VOnUpdate				- Chapter 19, page 730
//
void RevolverWarsHumanView::VOnUpdate(unsigned long deltaMs)
{
	HumanView::VOnUpdate( deltaMs );
	...
	if (m_pRevolverController)
	{
		m_pRevolverController->OnUpdate(deltaMs);
	}
...
//
// RevolverWarsHumanView::VOnAttach				- Chapter 19, page 731
//
void RevolverWarsHumanView::VOnAttach(GameViewId vid, ActorId aid)
{
	HumanView::VOnAttach(vid, aid);
}

bool RevolverWarsHumanView::VLoadGameDelegate(TiXmlElement* pLevelData)
{
...
void RevolverWarsHumanView::VSetControlledActor(ActorId actorId) 
{ 
	m_pRevolver = static_pointer_cast<SceneNode>(m_pScene->FindActor(actorId));
    if (!m_pRevolver)
    {
        GCC_ERROR("Invalid Revolver");
        return;
    }

	HumanView::VSetControlledActor(actorId);

    m_pRevolverController.reset(GCC_NEW RevolverController(m_pRevolver));
    m_KeyboardHandler = m_pRevolverController;
    m_PointerHandler = m_pRevolverController;
    m_pCamera->SetTarget(m_pRevolver);
    m_pRevolver->SetAlpha(0.8f);
...
void RevolverWarsHumanView::GameplayUiUpdateDelegate(IEventDataPtr pEventData)
{
...
void RevolverWarsHumanView::SetControlledActorDelegate(IEventDataPtr pEventData)
{
...
void RevolverWarsHumanView::DestroyPlayerDelegate(IEventDataPtr pEventData)
{
...
void RevolverWarsHumanView::RegisterAllDelegates(void)
{
    ...
    pGlobalEventManager->VAddListener(MakeDelegate(this, &RevolverWarsHumanView::GameplayUiUpdateDelegate), EvtData_Gameplay_UI_Update::sk_EventType);
    pGlobalEventManager->VAddListener(MakeDelegate(this, &RevolverWarsHumanView::SetControlledActorDelegate), EvtData_Set_Controlled_Actor::sk_EventType);
	//pGlobalEventManager->VAddListener(MakeDelegate(this, &RevolverWarsHumanView::SetControlledActorDelegate), EvtData_Set_Controlled_Actor::sk_EventType);
	pGlobalEventManager->VAddListener(MakeDelegate(this, &RevolverWarsHumanView::DestroyPlayerDelegate), EvtData_Destroy_Player::sk_EventType);
}

void RevolverWarsHumanView::RemoveAllDelegates(void)
{
    IEventManager* pGlobalEventManager = IEventManager::Get();
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &RevolverWarsHumanView::GameplayUiUpdateDelegate), EvtData_Gameplay_UI_Update::sk_EventType);
    pGlobalEventManager->VRemoveListener(MakeDelegate(this, &RevolverWarsHumanView::SetControlledActorDelegate), EvtData_Set_Controlled_Actor::sk_EventType);
	//pGlobalEventManager->VRemoveListener(MakeDelegate(this, &RevolverWarsHumanView::SetControlledActorDelegate), EvtData_Set_Controlled_Actor::sk_EventType);
	pGlobalEventManager->VRemoveListener(MakeDelegate(this, &RevolverWarsHumanView::DestroyPlayerDelegate), EvtData_Destroy_Player::sk_EventType);
}
...
///////////////////////////////////////////////////////////////////////////////
//
// AIRevolverView::AIRevolverView					- Chapter 19, page 737
//
AIRevolverView::AIRevolverView(shared_ptr<PathingGraph> pPathingGraph) : IGameView(), m_pPathingGraph(pPathingGraph)
{
    //
}
...
// AIRevolverView::~AIRevolverView					- Chapter 19, page 737
//
AIRevolverView::~AIRevolverView(void)
{
    GCC_LOG("AI", "Destroying AIRevolverView");
}
